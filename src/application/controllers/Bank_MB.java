...
public class Bank_MB {
	...
	@FXML private TextField MBInput;
	...
	void StartDateInputHandler(KeyEvent event) {
		if (StartDateinput == null || MBInput == null) return;
	...
	void NameInputHandler(KeyEvent event) {
		...
		if (MBInput.getLength() >= 30 && !(event.getCode().toString().equals("BACK_SPACE"))) {
			event.consume();
		}
		if (MBInput.getLength() >= 30) {
