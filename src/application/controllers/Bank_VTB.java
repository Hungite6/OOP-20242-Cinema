...
public class Bank_VTB {
	...
	@FXML private TextField VTBInput;
	...
	void StartDateInputHandler(KeyEvent event) {
		if (StartDateinput == null || VTBInput == null) return;
		...
		if (text.length() == 5) {
			VTBInput.requestFocus();
		}
	}

	@FXML
	void NameInputHandler(KeyEvent event) {
		if (VTBInput == null) return; 

		if (VTBInput.getLength() >= 30 && !(event.getCode().toString().equals("BACK_SPACE"))) {
			event.consume();
		}
		if (VTBInput.getLength() >= 30) {
