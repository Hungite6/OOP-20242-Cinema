...
public class Bank_BIDV {
	...
	@FXML private TextField BIDVInput;
	...
	void PaybuttonHandler(ActionEvent event) throws IOException {
		if (CreditCardInput.getLength() != 19 ||
			...
			BIDVInput.getText().trim().isEmpty()) {
	...
	void StartDateInputHandler(KeyEvent event) {
		if (StartDateinput == null || BIDVInput == null) return;
		...
		if (text.length() == 5) {
			BIDVInput.requestFocus();
		}
	}

	@FXML
	void NameInputHandler(KeyEvent event) {
		if (BIDVInput == null) return; 

		if (BIDVInput.getLength() >= 30 && !(event.getCode().toString().equals("BACK_SPACE"))) {
			event.consume();
		}
		if (BIDVInput.getLength() >= 30) {
